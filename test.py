...
with tab3:
    st.subheader("Tren Rata-rata Penyewaan Sepeda per Jam pada Hari Kerja dan Hari Libur/Weekend")
    ...
    plt.title('Perbandingan Rata-Rata Banyak Penyewaan Sepeda per Jam', size=20)
    ...
    st.subheader("Perbandingan Rata-rata Penyewaan per Jam antara Hari Kerja dan Hari Libur/Weekend")
    ...
    plt.title(f'Perbandingan Rata-Rata Banyak Penyewaan Sepeda pada Jam {selected_hour}', size=20)
    plt.ylabel('Banyak Penyewaan', size=12)
...
with tab4 :
    st.subheader("Tren Rata-rata Penyewaan Per Jam Berdasarkan Kategori Musim")
    ...
    plt.title(f'Rata-rata Banyak Penyewaan Sepeda per Jam pada {selected_season}')
    plt.xticks(range(24))

    # Tampilkan plot di Streamlit
    st.pyplot(plt)

    st.subheader("Perbandingan Rata-rata Penyewaan per Jam Berdasarkan Kategori Musim")
    ...
    plt.xlabel('Musim')
    plt.ylabel('Rata-rata Banyak Penyewaan per Jam')
    plt.title(f'Rata-rata Banyak Penyewaan Sepeda pada Jam {selected_hour2} Berdasarkan Kategori Musim')
    
    # Tambahkan label di atas setiap bar
    for i in range(len(avg_by_season)):
        plt.text(i, avg_by_season.values[i] -1, round(avg_by_season.values[i], 2), ha='center',va="center" )

    # Tampilkan plot di Streamlit
    st.pyplot(plt)

    st.subheader("Tren Rata-rata Penyewaan per Jam Berdasarkan Kategori Cuaca")
    ...
    plt.xlabel('Jam')
    plt.ylabel('Rata-rata Banyak Penyewaan per Jam')
    plt.title(f'Rata-rata Banyak Penyewaan Sepeda per Jam pada Cuaca {selected_weather}')
    plt.xticks(range(24))

    # Tampilkan plot di Streamlit
    st.pyplot(plt)

    st.subheader("Perbandingan Rata-rata Penyewaan per Jam Berdasarkan Kategori Cuaca")
    # Membuat kolom 'season_category' untuk kategori musim
    ...
    plt.xlabel('Musim')
    plt.ylabel('Rata-rata Banyak Penyewaan per Jam')
    plt.title(f'Rata-rata Banyak Penyewaan Sepeda pada Jam {selected_hour3} Berdasarkan Kategori Cuaca')
    
    # Tambahkan label di atas setiap bar
    for i in range(len(avg_by_weather)):
        plt.text(i, avg_by_weather.values[i] - 1, round(avg_by_weather.values[i], 2), ha='center', va='center')
